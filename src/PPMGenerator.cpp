...
PPMGenerator::PPMGenerator(Status* statusPtr, uint8_t defaultPrio, TIM_HandleTypeDef* htim, float* controllerValueX, float* controllerValueY) :
...
void PPMGenerator::update() {
	...
	if( status->rcSignalEnable ) {
		...
		if( status->rcSignalThrottle < THROTTLE_MIN ) {
			status->motorValues[0] = status->rcSignalThrottle + controllerValueX;
			status->motorValues[1] = status->rcSignalThrottle + controllerValueY;
			status->motorValues[2] = status->rcSignalThrottle - controllerValueX;
			status->motorValues[3] = status->rcSignalThrottle - controllerValueY;
